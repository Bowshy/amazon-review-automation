...
The application uses Drizzle ORM with PostgreSQL and is optimized for Supabase hosting. The configuration includes:
...
- Automatically recreates database client
...
3. **Migration Failures**
   ```
   Drizzle migration failed to connect
   ```
...
```javascript
import { getConnectionInfo, getQueryStats } from '$lib/db/config';
...
1. Export schema using Drizzle introspection
